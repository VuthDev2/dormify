...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Determine redirect based on subscription plan
      const { data: { user } } = await supabase.auth.getUser();
      if (user) {
        // Check if user is a tenant (not an admin)
          ...
          <div className="bg-slate-50/60 border border-border/50 shadow-xl shadow-black/[0.02] rounded-[2rem] p-6 sm:p-10 space-y-8">
            ...
            <form onSubmit={handleLogin} className="space-y-5">
              <div className="space-y-3.5">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex justify-end pt-2">
                    <Link href="/forgot" className="text-xs font-bold text-blue-600 hover:text-blue-700 transition-colors">
                      Forgot Password?
                    </Link>
                  </div>
                </div>
              ...
              <Button
                ...
                variant="outline"
                disabled
                className="w-full h-12 rounded-xl border-border/40 bg-white shadow-sm hover:bg-slate-50 font-semibold text-sm flex items-center justify-center gap-3 transition-all opacity-50 cursor-not-allowed"
              >
